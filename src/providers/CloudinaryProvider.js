...
// Extract only URLs generated by this Cloudinary account. External/default avatar URLs are ignored.
const getPublicIdFromUrl = (url) => {
  ...
  try {
    ...
    const segments = decodeURIComponent(parsedUrl.pathname).split('/').filter(Boolean)
    if (segments[0] !== env.CLOUDINARY_CLOUD_NAME) return null
    const uploadIndex = segments.indexOf('upload')
    const versionIndex = segments.findIndex((segment, index) => index > uploadIndex && /^v\d+$/.test(segment))
    if (uploadIndex < 0 || versionIndex < 0 || versionIndex === segments.length - 1) return null
